...
export function RevenueExpenseAnalyticsCenter({ model }: { model: FinanceVisualFinalizationModel }) {
  const { t } = useTranslation();
  return (
    <section className="grid gap-4 lg:grid-cols-3">
      <article className="card p-4 lg:col-span-2">
        <h3 className="mb-3 text-sm font-semibold text-darktext">
          {t("finance.visual.analytics.revenueExpense", "Revenue & expense analytics")}
        </h3>
        <DashboardChartFromSpec
          chart={model.chart}
          emptyMessage={t("finance.visual.chart.empty", "No financial series available yet.")}
        />
      </article>
      <article className="card p-4">
        <h3 className="text-sm font-semibold text-darktext">
          {t("finance.visual.analytics.anomalies", "Anomaly finance detection")}
        </h3>
        <ul className="mt-3 space-y-2">
          ...
          {model.alerts.length === 0 ? (
            <li className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-500">
              {t("finance.visual.analytics.noAlerts", "No anomalies detected.")}
            </li>
          ) : null}
        </ul>
      </article>
