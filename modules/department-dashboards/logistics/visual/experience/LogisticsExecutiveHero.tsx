...
const ICONS = [Package, Warehouse, Truck, Package];

export function LogisticsExecutiveHero({ model }: { model: LogisticsVisualFinalizationModel }) {
  const { t } = useTranslation();
  return (
    <section className="card border border-emerald-200 bg-gradient-to-r from-emerald-50 to-white p-5">
      <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">
        {t("logistics.visual.hero.kicker", "Supply chain operations center")}
      </p>
      <h2 className="mt-1 text-xl font-bold text-darktext">
        {t("logistics.visual.hero.title", "Logistics executive intelligence")}
      </h2>
      <p className="mt-1 text-sm text-gray-600">
        {t("logistics.visual.hero.subtitle", "Inventory, warehouses, deliveries and supplier signals in one control center.")}
      </p>
