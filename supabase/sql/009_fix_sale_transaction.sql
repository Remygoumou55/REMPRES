-- =============================================================================
-- RemPres ERP — Migration 009
-- Fix architecture : create_sale_transaction utilise record_financial_transaction
--
-- AVANT : create_sale_transaction avait son propre INSERT dans financial_transactions
-- APRÈS : il délègue à record_financial_transaction (source unique de vérité)
--
-- Avantage : toute évolution de la logique financière se fait dans UN seul endroit.
-- =============================================================================
...
begin
  ...
  if p_payment_method is null or p_payment_method not in ('cash','mobile_money','bank_transfer') then
    raise exception using errcode='P0001', message='INVALID_PAYMENT_METHOD',
      detail=format('Mode de paiement invalide : "%s". Valeurs : cash, mobile_money, bank_transfer.', p_payment_method);
  end if;
  ...
  -- ══════════════════════════════════════════════════════════════════════════
  -- ÉTAPE 1 : Verrouillage + validation par article
  -- ══════════════════════════════════════════════════════════════════════════
  for v_item in
  ...
  -- ══════════════════════════════════════════════════════════════════════════
  -- ÉTAPE 5 : Enregistrement financier via record_financial_transaction
  --
  -- ARCHITECTURE FIX : on délègue à la fonction centralisée.
  -- Plus de duplication de logique → source unique de vérité.
  -- PERFORM = appel de fonction sans récupérer la valeur de retour.
  -- ══════════════════════════════════════════════════════════════════════════
